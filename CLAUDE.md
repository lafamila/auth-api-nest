...
3. **계획 → 구현 → 검토** — 계획 단계에서 검토 통과 기준(어떤 테스트/명령이 통과해야 "done"인지)을 명시한다. auth 관련 변경은 *반드시* 머지 전 테스트.
4. **Docker 빌드 가능** — DEPLOY 예정. Phase 1 구현 시점에 Dockerfile + 루트 `docker-compose.yml` 등록 필요. issuer 도메인은 `https://auth.lafamila.xyz`.
