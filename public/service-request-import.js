(function attachServiceRequestImport(globalObject) {
  ...
  const SUPPORTED_CREDENTIAL_SCOPES = new Set(['account.search', 'permission.read']);
  ...
  function normalizePermissionRows(value, warnings) {
    ...
    value.forEach(function eachPermission(permission, index) {
      ...
      const key = ensureOptionalString(row.key, 'permissions[' + index + '].key');
      const label = ensureOptionalString(row.label, 'permissions[' + index + '].label');
  ...
  function normalizeOidcClientRows(value) {
    ...
    return value.map(function mapClient(client, index) {
      ...
      const clientId = ensureOptionalString(row.clientId, 'oidcClients[' + index + '].clientId');
      ...
      if (!SUPPORTED_CLIENT_TYPES.has(clientType)) {
        throw new Error(
          'oidcClients[' + index + '].clientType must be public or confidential.',
        );
      }
      ...
      if (row.requirePkce !== undefined && row.requirePkce !== null) {
        if (typeof row.requirePkce !== 'boolean') {
          throw new Error('oidcClients[' + index + '].requirePkce must be a boolean.');
        }
  ...
  function normalizeServiceCredentialRows(value) {
    if (value === undefined || value === null) {
      return [];
    }
    if (!Array.isArray(value)) {
    ...
    return value.map(function mapCredential(credential, index) {
      const row = ensurePlainObject(credential, 'serviceCredentials[' + index + ']');
      const name = ensureOptionalString(row.name, 'serviceCredentials[' + index + '].name');
      const description = ensureOptionalString(
        row.description,
  ...
  function normalizeImportedServiceRequest(value, sessionAccount) {
    ...
    if (unknownFields.length) {
      warnings.push('Ignored unknown top-level fields: ' + unknownFields.join(', ') + '.');
    }
    const sessionName =
      sessionAccount && typeof sessionAccount.name === 'string' ? sessionAccount.name : '';
    const sessionEmail =
      sessionAccount && typeof sessionAccount.email === 'string' ? sessionAccount.email : '';
    ...
    return {
      draft: {
        ...
        serviceCredentials: normalizeServiceCredentialRows(root.serviceCredentials),
      },
      warnings: warnings,
    };
  }

  function parseImportedServiceRequestText(text, sessionAccount) {
    let parsed;
    try {
      parsed = JSON.parse(text);
    } catch (error) {
      throw new Error('Invalid JSON file. Check the file contents and try again.');
    }
    return normalizeImportedServiceRequest(parsed, sessionAccount);
  }
