...
7. 문서를 업데이트한다.
   ...
   - `docs/service-integration.md`: API 직접 제출과 admin UI 제출의 차이, `requestSecret` 과 운영 secret 의 차이를 정리한다.
...
- admin UI 에서 승인 시 표시되는 운영 secret 흐름과 요청 생성 시 표시되는 `requestSecret` 흐름이 구분되는지 보고한다.
